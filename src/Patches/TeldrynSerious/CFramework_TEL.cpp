...
namespace CPatch_TEL 
{
	...
	void CHandler::InjectAndCompileData() {
		...
		ItemData.CompileFormArray(CPatch_TEL::Items, modname);
		BookData.CompileFormArray(CPatch_TEL::Books, modname);
		MapsData.CompileFormArray(CPatch_TEL::MapMa, modname);
	...
	void CHandler::InstallSearchTerms()
	{
		for (auto i = 0; i < Items_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(Items_FormArray[i], Items_NameArray[i], "$MCMPageTeldrynSerious", std::to_underlying(EntryCategory::kItem)));
		}
		for (auto i = 0; i < Books_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(Books_FormArray[i], Books_NameArray[i], "$MCMPageTeldrynSerious", FrameworkAPI::GetBookCategoryType(Books_FormArray[i])));
		}
		for (auto i = 0; i < MapMa_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(MapMa_FormArray[i], MapMa_NameArray[i], "$MCMPageTeldrynSerious", std::to_underlying(EntryCategory::kMapM)));
		}
	}
