			...
			if (auto t_pos = std::ranges::find(ItmL1_NameArray, a_name); t_pos != ItmL1_NameArray.end()) {
				...
				if (ItmL1_BoolArray.at(b_pos)) {
					...
					for (auto var : CPatch_ST2_ItmL1::Data.GetAllVariations()) {
						if (CPatch_ST2_ItmL1::Data.GetBase(var) == ItmL1_FormArray.at(b_pos)->GetFormID()) {
			...
			if (auto t_pos = std::ranges::find(ItmL2_NameArray, a_name); t_pos != ItmL2_NameArray.end()) {
				...
				if (ItmL2_BoolArray.at(b_pos)) {
					...
					for (auto var : CPatch_ST2_ItmL2::Data.GetAllVariations()) {
						if (CPatch_ST2_ItmL2::Data.GetBase(var) == ItmL2_FormArray.at(b_pos)->GetFormID()) {
							FoundItemData.RemoveForm(var);
						}
					}
				}
			...
			if (auto t_pos = std::ranges::find(ItmL3_NameArray, a_name); t_pos != ItmL3_NameArray.end()) {
				...
				if (ItmL3_BoolArray.at(b_pos)) {
					...
					for (auto var : CPatch_ST2_ItmL3::Data.GetAllVariations()) {
						if (CPatch_ST2_ItmL3::Data.GetBase(var) == ItmL3_FormArray.at(b_pos)->GetFormID()) {
							FoundItemData.RemoveForm(var);
						}
					}
				}
			...
			if (auto t_pos = std::ranges::find(ItmL4_NameArray, a_name); t_pos != ItmL4_NameArray.end()) {
				...
				if (ItmL4_BoolArray.at(b_pos)) {
					...
					for (auto var : CPatch_ST2_ItmL4::Data.GetAllVariations()) {
						if (CPatch_ST2_ItmL4::Data.GetBase(var) == ItmL4_FormArray.at(b_pos)->GetFormID()) {
							FoundItemData.RemoveForm(var);
						}
					}
				}
