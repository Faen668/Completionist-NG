	...
	void CHandler::ProcessFoundForm(RE::FormID a_baseID, RE::FormID a_eventID, std::string a_variable) {
		...
		if (a_variable == "NotifyBooks") {
			...
			if (!FoundItemData.HasForm(a_eventID)) {
				...
				if (auto* book = static_cast<RE::TESObjectBOOK*>(CPatch_BOO_Books::Data.GetForm(a_eventID)); book && book->GetSpell()) {
					FrameworkAPI::AddNewEventToLog(Serialization::CompletionistLog::kTome, book->GetName());
				}
				else {
					FrameworkAPI::AddNewEventToLog(Serialization::CompletionistLog::kBook, CPatch_BOO_Books::Data.GetForm(a_eventID)->GetName());
				}
			}
		...
		if (a_variable == "NotifyItems") {
			...
			if (!FoundItemData.HasForm(a_eventID)) {
				...
				FrameworkAPI::SendNotification(msg, a_variable);
				FrameworkAPI::AddNewEventToLog(Serialization::CompletionistLog::kCollected, CPatch_BOO_Items::Data.GetForm(a_eventID)->GetName());
			}
