...
namespace CPatch_FOS {
	...
	void CHandler::OnContainerChangedEvent(RE::TESContainerChangedEvent const* a_event) {
		...
		if (ItemData2.HasForm(a_event->baseObj)) {
			...
			CHandler::ProcessFoundForm(base, a_event->baseObj, ItemData1, ItmL2_FormArray, &ItmL2_BoolArray, &ItmL2_EntriesFound, cmd::kCollected, "NotifyItems");
			return;
		}
		...
		if (ItemData3.HasForm(a_event->baseObj)) {
			...
			CHandler::ProcessFoundForm(base, a_event->baseObj, ItemData1, ItmL3_FormArray, &ItmL3_BoolArray, &ItmL3_EntriesFound, cmd::kCollected, "NotifyItems");
			return;
		}
	}
	...
	void CHandler::InjectAndCompileData() {
		...
		ItemData1.CompileFormArray(CPatch_FOS::ItmL1, modname);
		ItemData2.CompileFormArray(CPatch_FOS::ItmL2, modname);
		ItemData3.CompileFormArray(CPatch_FOS::ItmL3, modname);
	...
	void CHandler::InstallSearchTerms()
	{
		for (auto i = 0; i < ItmL1_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(ItmL1_FormArray[i], ItmL1_NameArray[i], "$MCMPageFossilMining1", std::to_underlying(EntryCategory::kItem)));
		}
		for (auto i = 0; i < ItmL2_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(ItmL2_FormArray[i], ItmL2_NameArray[i], "$MCMPageFossilMining2", std::to_underlying(EntryCategory::kItem)));
		}
		for (auto i = 0; i < ItmL3_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(ItmL3_FormArray[i], ItmL3_NameArray[i], "$MCMPageFossilMining3", std::to_underlying(EntryCategory::kItem)));
		}
	}
