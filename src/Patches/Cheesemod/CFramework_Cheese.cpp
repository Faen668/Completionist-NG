...
namespace CPatch_CHM {
	...
	void CHandler::OnContainerChangedEvent(RE::TESContainerChangedEvent const* a_event) {
		...
		if (ItemData1.HasForm(a_event->baseObj)) {
			...
			CHandler::ProcessFoundForm(base, a_event->baseObj, ItemData1, ItmL1_FormArray, &ItmL1_BoolArray, &ItmL1_EntriesFound, cmd::kCollected, "NotifyItems");
			return;
		}
		...
		if (ItemData2.HasForm(a_event->baseObj)) {
			...
			CHandler::ProcessFoundForm(base, a_event->baseObj, ItemData2, ItmL2_FormArray, &ItmL2_BoolArray, &ItmL2_EntriesFound, cmd::kCollected, "NotifyItems");
			return;
		}
		...
		if (ItemData3.HasForm(a_event->baseObj)) {
			...
			CHandler::ProcessFoundForm(base, a_event->baseObj, ItemData3, ItmL3_FormArray, &ItmL3_BoolArray, &ItmL3_EntriesFound, cmd::kCollected, "NotifyItems");
			return;
		}
		...
		if (ItemData4.HasForm(a_event->baseObj)) {
			...
			CHandler::ProcessFoundForm(base, a_event->baseObj, ItemData4, ItmL4_FormArray, &ItmL4_BoolArray, &ItmL4_EntriesFound, cmd::kCollected, "NotifyItems");
			return;
		}
		return;
	}
	...
	void CHandler::InjectAndCompileData() {
		...
		ItemData1.CompileFormArray(CPatch_CHM::ItmL1, modname);
		ItemData2.CompileFormArray(CPatch_CHM::ItmL2, modname);
		ItemData3.CompileFormArray(CPatch_CHM::ItmL3, modname);
		ItemData4.CompileFormArray(CPatch_CHM::ItmL4, modname);
	...
	void CHandler::InstallSearchTerms()
	{
		for (auto i = 0; i < ItmL1_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(ItmL1_FormArray[i], ItmL1_NameArray[i], "$MCMPageCheese4", std::to_underlying(EntryCategory::kItem)));
		}
		for (auto i = 0; i < ItmL2_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(ItmL2_FormArray[i], ItmL2_NameArray[i], "$MCMPageCheese1", std::to_underlying(EntryCategory::kItem)));
		}
		for (auto i = 0; i < ItmL3_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(ItmL3_FormArray[i], ItmL3_NameArray[i], "$MCMPageCheese2", std::to_underlying(EntryCategory::kItem)));
		}
		for (auto i = 0; i < ItmL4_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(ItmL4_FormArray[i], ItmL4_NameArray[i], "$MCMPageCheese3", std::to_underlying(EntryCategory::kItem)));
		}
	}
