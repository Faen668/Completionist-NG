...
namespace CPatch_GCN 
{
	...
	void CHandler::InstallQuestFramework()
	{
		for (auto i = 0; i < std::extent_v<decltype(QuestData)>; i++)
		{
			...
			CQuestMaster::CQuestDataVec.push_back(std::make_tuple(&QuestData[i], QuestData[i].GetName(), 40, QuestData[i].unique_identifier));
		}
	...
	void CHandler::ProcessFoundForm(RE::FormID a_baseID, RE::FormID a_eventID, std::string a_variable) {
		...
		if (a_variable == "NotifyBooks") {
			...
			if (!FoundItemData.HasForm(a_eventID)) {
				...
				if (auto* book = static_cast<RE::TESObjectBOOK*>(CPatch_GCN_Books::Data.GetForm(a_eventID)); book && book->GetSpell()) {
					FrameworkAPI::AddNewEventToLog(Serialization::CompletionistLog::kTome, book->GetName());
				}
				else {
					FrameworkAPI::AddNewEventToLog(Serialization::CompletionistLog::kBook, CPatch_GCN_Books::Data.GetForm(a_eventID)->GetName());
				}
			}
		...
		if (a_variable == "NotifyItems") {
			...
			if (!FoundItemData.HasForm(a_eventID)) {
				...
				FrameworkAPI::SendNotification(msg, a_variable);
				FrameworkAPI::AddNewEventToLog(Serialization::CompletionistLog::kCollected, CPatch_GCN_Items::Data.GetForm(a_eventID)->GetName());
			}
