...
namespace CPatch_CLW {
	...
	void CHandler::InstallQuestFramework()
	{
		for (auto i = 0; i < std::extent_v<decltype(QuestData)>; i++)
		{
			...
			CQuestMaster::CQuestDataVec.push_back(std::make_tuple(&QuestData[i], QuestData[i].GetName(), 32, QuestData[i].unique_identifier));
		}
	};
	...
	void CHandler::InjectAndCompileData() {
		...
		ItemData.CompileFormArray(CPatch_CLW::Items, modname);
		BookData.CompileFormArray(CPatch_CLW::Books, modname);
		MapsData.CompileFormArray(CPatch_CLW::MapMa, modname);
	...
	void CHandler::InstallSearchTerms()
	{
		for (auto i = 0; i < Items_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(Items_FormArray[i], Items_NameArray[i], "$MCMPageClockwork", std::to_underlying(EntryCategory::kItem)));
		}
		for (auto i = 0; i < Books_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(Books_FormArray[i], Books_NameArray[i], "$MCMPageClockwork", FrameworkAPI::GetBookCategoryType(Books_FormArray[i])));
		}
		for (auto i = 0; i < MapMa_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(MapMa_FormArray[i], MapMa_NameArray[i], "$MCMPageClockwork", std::to_underlying(EntryCategory::kMapM)));
		}
	}
