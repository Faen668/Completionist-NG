...
namespace CPatch_HRB 
{
	...
	void CHandler::InstallQuestFramework()
	{
		for (auto i = 0; i < std::extent_v<decltype(QuestData)>; i++)
		{
			...
			CQuestMaster::CQuestDataVec.push_back(std::make_tuple(&QuestData[i], QuestData[i].GetName(), 34, QuestData[i].unique_identifier));
		}
	...
	void CHandler::ProcessFoundForm(RE::FormID a_baseID, RE::FormID a_eventID, std::string a_variable) {
		...
		if (a_variable == "NotifyBooks") {
			...
			if (!FoundItemData.HasForm(a_eventID)) {
				...
				if (auto* book = static_cast<RE::TESObjectBOOK*>(CPatch_HRB_Books::Data.GetForm(a_eventID)); book && book->GetSpell()) {
					FrameworkAPI::AddNewEventToLog(Serialization::CompletionistLog::kTome, book->GetName());
				}
				else {
					FrameworkAPI::AddNewEventToLog(Serialization::CompletionistLog::kBook, CPatch_HRB_Books::Data.GetForm(a_eventID)->GetName());
				}
			}
		...
		if (a_variable == "NotifyItems") {
			...
			if (!FoundItemData.HasForm(a_eventID)) {
				...
				FrameworkAPI::SendNotification(msg, a_variable);
				FrameworkAPI::AddNewEventToLog(Serialization::CompletionistLog::kCollected, CPatch_HRB_Items::Data.GetForm(a_eventID)->GetName());
			}
	...
	void CHandler::InstallSearchTerms()
	{
		for (auto& name : Items_NameArray) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(name, "$MCMPageHelgen", std::to_underlying(EntryCategory::kItem)));
		}
		for (auto i = 0; i < Books_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(Books_NameArray[i], "$MCMPageHelgen", FrameworkAPI::GetBookCategoryType(Books_FormArray[i])));
		}
		for (auto& name : MapMa_NameArray) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(name, "$MCMPageHelgen", std::to_underlying(EntryCategory::kMapM)));
		}
	}
