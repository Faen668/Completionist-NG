...
namespace CPatch_HRB 
{
	...
	void CHandler::InjectAndCompileData() {
		...
		ItemData.CompileFormArray(CPatch_HRB::Items, modname);
		BookData.CompileFormArray(CPatch_HRB::Books, modname);
		MapsData.CompileFormArray(CPatch_HRB::MapMa, modname);
	...
	void CHandler::InstallSearchTerms()
	{
		for (auto i = 0; i < Items_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(Items_FormArray[i], Items_NameArray[i], "$MCMPageHelgen", std::to_underlying(EntryCategory::kItem)));
		}
		for (auto i = 0; i < Books_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(Books_FormArray[i], Books_NameArray[i], "$MCMPageHelgen", FrameworkAPI::GetBookCategoryType(Books_FormArray[i])));
		}
		for (auto i = 0; i < MapMa_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(MapMa_FormArray[i], MapMa_NameArray[i], "$MCMPageHelgen", std::to_underlying(EntryCategory::kMapM)));
		}
	}
