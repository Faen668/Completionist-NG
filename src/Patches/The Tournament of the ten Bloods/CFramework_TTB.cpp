...
namespace CPatch_TTB 
{
	...
	constexpr std::string_view modname = "ArtifactsOfBoethiah.esp";
	...
	void CHandler::InjectAndCompileData() {
		...
		ItemData.CompileFormArray(CPatch_TTB::Items, modname);
		ItemData.CompileVariation(CPatch_TTB::ItemsV, modname);

		BookData.CompileFormArray(CPatch_TTB::Books, modname);
		MapsData.CompileFormArray(CPatch_TTB::MapMa, modname);
	...
	void CHandler::InstallSearchTerms()
	{
		for (auto i = 0; i < Items_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(Items_FormArray[i], Items_NameArray[i], "$MCMPageTTB", std::to_underlying(EntryCategory::kItem)));
		}
		for (auto i = 0; i < Books_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(Books_FormArray[i], Books_NameArray[i], "$MCMPageTTB", FrameworkAPI::GetBookCategoryType(Books_FormArray[i])));
		}
		for (auto i = 0; i < MapMa_NameArray.size(); i++) {
			CFramework_Master::CItemsDataVec.push_back(std::make_tuple(MapMa_FormArray[i], MapMa_NameArray[i], "$MCMPageTTB", std::to_underlying(EntryCategory::kMapM)));
		}
	}
